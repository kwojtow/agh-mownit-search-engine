...
def prepare_dictionary():
    ...
    wordfreq = {}
    for url in urls:
        print("Preparing dictionary: ", url)
        tokens = scrap(url)
        for token in tokens:
...
def prepare_bag_of_words():
    ...
    art_vec_list = [[] for _ in range(len(urls))]

    def calc_article(i):
        url = urls[i]
        global articles_vectors
        print("Preparing bag-of-words: ", url)
        article_tokens = scrap(url)
        for token in dictionary:
            art_vec_list[i].append(article_tokens.count(token))

    threads = [0 for _ in range(12)]
    for i in range(len(urls)):
        threads[i % 12] = threading.Thread(target=calc_article, args=(i, ))
        threads[i % 12].start()
        if i % 12 == 11 or i == len(urls) - 1:
            for j in range(i % 12 + 1):
                threads[j].join()

    for art_vec in art_vec_list:
...
k_val = np.min([articles_vectors.shape[1], articles_vectors.shape[0]]) // 10
